...
- server adapter에서 자주 쓰는 204, 400, 401, 403, 404, 409, 422, 502 response helper가 필요할 때 사용합니다
...
| `noContent` | body가 없는 204 response를 반환합니다 |
| `badRequest` | 400 message response를 반환합니다. |
| `unauthorized` | 401 message response를 반환합니다. |
| `forbidden`, `notFound` | 403, 404 message response를 반환합니다 |
| `conflict`, `unprocessableEntity` | 409, 422 message response를 반환합니다 |
...
- 400, 401, 403, 404, 409, 422, 502 shortcut은 `{ message }` JSON을 반환합니다
- `noContent`는 body가 없는 204 response를 반환합니다
- 그 외 status는 `messageResponse`를 사용합니다
